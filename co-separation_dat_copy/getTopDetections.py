...
detection_results_root = 'detection_results_huy/MUSIC_solo/top_detections_two_classes' #where the .npy object detection files are saved
top_detections_root = 'top_detections_huy/MUSIC_solo/top_detections_two_classes'
os.makedirs(top_detections_root,exist_ok=True)
...
for detection in cliplist:
	...
	if results.shape[0] >= 60 and results.shape[0] <= 300:
		print(detection)
		path = detection.replace("detection_results_huy","top_detections_huy")
