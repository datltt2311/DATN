...
def main():
	...
	if opt.with_additional_scene_image:
	    opt.number_of_classes = opt.number_of_classes
	...
	vid1 = opt.video1_name.split("/")
	if "duet" in opt.video1_name:
		vid1_path =  "/ext_data2/manhnh/MUSIC_dataset/reshape_11025/duet/top_detections_two_classes/"
	else:
		vid1_path = "/ext_data2/manhnh/MUSIC_dataset/reshape_11025/MUSIC_solo/top_detections_two_classes/"
	audio1_path = vid1_path + vid1[7] + ".wav"
	audio1, _ = librosa.load(audio1_path, sr=opt.audio_sampling_rate)
	#audio2_path = opt.video2_name.replace('/top_detections_new/','/reshape_11025_new/')
	vid2 = opt.video2_name.split("/")
	if "duet" in opt.video2_name:
		vid2_path =  "/ext_data2/manhnh/MUSIC_dataset/reshape_11025/duet/top_detections_two_classes/"
	else:
		vid2_path = "/ext_data2/manhnh/MUSIC_dataset/reshape_11025/MUSIC_solo/top_detections_two_classes/" #audio2_path = audio2_path.replace('/top_detections/','/reshape_11025/')
	#audio2_path = "/ext_data2/students/hainghiem/co-separation/audio_11025/" + opt.video2_name + ".wav"
	audio2_path = vid2_path + vid2[7] + ".wav" # audio2_path = os.path.join(opt.data_path, 'reshape_11025', opt.video2_name + '.wav')
	...
	detectionResult1 = np.load(opt.video1_name.replace("/ext_data2/manhnh/MUSIC_dataset/output_img", "/ext_data2/students/hainghiem/faster-rcnn.pytorch-pytorch-1.0/detection_results") + ".npy") #/ext_data2/students/hainghiem/faster-rcnn.pytorch-pytorch-1.0/top_detections/" + vid1[7] + '.npy')
	detectionResult2 = np.load(opt.video2_name.replace("/ext_data2/manhnh/MUSIC_dataset/output_img", "/ext_data2/students/hainghiem/faster-rcnn.pytorch-pytorch-1.0/detection_results") + ".npy") #"/ext_data2/students/hainghiem/faster-rcnn.pytorch-pytorch-1.0/top_detections/" + vid2[7] + '.npy')
	...
	for i in range(opt.num_of_object_detections_to_use):
		...
		frame1_PATH = opt.video1_name #frame1_PATH = opt.video1_name.replace('/top_detections_new/','/frame_new/')
		#frame1_PATH = "/ext_data2/students/hainghiem/co-separation/frame/" + opt.video1_name
		frame2_PATH = opt.video2_name #frame2_PATH = opt.video2_name.replace('/top_detections_new/','/frame_new/')
		#frame2_PATH = "/ext_data2/students/hainghiem/co-separation/frame/" + opt.video2_name
		frame_path1 = os.path.join(frame1_PATH, "%06d.png" % det_box1[0]) #"/ext_data2/students/hainghiem/co-separation/white.png"
		frame_path2 = os.path.join(frame2_PATH, "%06d.png" % det_box2[0]) #"/ext_data2/students/hainghiem/co-separation/white.png"
		...
		while sliding_window_start + samples_per_window < audio_length:
			...
			#reconstructed_signal = outputs['audio_decode'].detach().cpu().numpy()[-1,-1,:65535]
			# print(reconstructed_signal.size())
	...
	output_dir = os.path.join(opt.output_dir_root, vid1[5] + '_' + vid1[7] + 'VS' + vid2[5] + '_' + vid2[7])
