...
def main():
	...
	#nets = (net_visual, net_unet, net_classifier, net_vocal)
	...
	#audio1_path = "/ext_data2/manhnh/MUSIC_dataset/reshape_11025_new/duet/top_detections_two_classes/--IXKwloYLE_0.1.wav"
	#audio1, _ = librosa.load(audio1_path, sr=opt.audio_sampling_rate)

	#load the two audios
	# audio1_path = os.path.join(opt.data_path, 'reshape_11025', opt.video1_name + '.wav')
	# audio_path = npy_path.replace("yolo_","")

	audio1_path = opt.video1_name.replace('yolo_','')
	audio1_path = audio1_path.replace('/top_detections_new/','/reshape_11025_new/')
	audio1_path = audio1_path.replace('/top_detections/','/reshape_11025/')
	audio1_path = audio1_path+".wav"
    #audio1_path = "/ext_data2/manhnh/MUSIC_dataset/reshape_11025_new/duet/top_detections_two_classes/--IXKwloYLE_0.1.wav"
	audio1, _ = librosa.load(audio1_path, sr=opt.audio_sampling_rate)

	audio2_path = opt.video2_name.replace('yolo_','')
	audio2_path = audio2_path.replace('/top_detections_new/','/reshape_11025_new/')
	audio2_path = audio2_path.replace('/top_detections/','/reshape_11025/')
	audio2_path = audio2_path+".wav"
	#audio2_path = '/ext_data2/manhnh/MUSIC_dataset/reshape_11025_new/duet/top_detections_two_classes/1xWqVxZNf1Q_1.9.wav'
	#audio2_path = os.path.join(opt.data_path, 'reshape_11025', opt.video2_name + '.wav')
	...
	detectionResult1 = np.load(opt.video1_name + '.npy')
	detectionResult2 = np.load(opt.video2_name + '.npy')
	
	pose_path = "/ext_data2/home/manhnguyen/new/st-gcn-alphapose/pose_feature"
	pose_1 = torch.from_numpy(np.load(os.path.join(pose_path, opt.video1_name.split('/')[-1] + '.npy')))
	pose_2 = torch.from_numpy(np.load(os.path.join(pose_path, opt.video2_name.split('/')[-1] + '.npy')))

	#detectionResult1 = np.load("/ext_data2/manhnh/MUSIC_dataset/yolo_top_detections_new/duet/top_detections_two_classes/--IXKwloYLE_0.1.npy") #('--IXKwloYLE_0.1' + '.npy')
	#detectionResult2 = np.load("/ext_data2/manhnh/MUSIC_dataset/yolo_top_detections_new/duet/top_detections_two_classes/1xWqVxZNf1Q_1.9.npy") #('1xWqVxZNf1Q_1.9' + '.npy')
	...
	for i in range(opt.num_of_object_detections_to_use):
		...
		frame1_PATH = opt.video1_name.replace('yolo_','')
		frame1_PATH = frame1_PATH.replace('/top_detections_new/','/frame_new/')
		frame1_PATH = frame1_PATH.replace('/top_detections/','/frame/')
		frame2_PATH = opt.video2_name.replace('yolo_','')
		frame2_PATH = frame2_PATH.replace('/top_detections_new/','/frame_new/')
		frame2_PATH = frame2_PATH.replace('/top_detections/','/frame/')
		#frame1_PATH = "/ext_data2/manhnh/MUSIC_dataset/output_img/duet/cello-acoustic_guitar/--IXKwloYLE_0/"
		#frame2_PATH = "/ext_data2/manhnh/MUSIC_dataset/output_img/duet/acoustic_guitar-violin/1xWqVxZNf1Q_1/"
		frame_path1 = os.path.join(frame1_PATH, "%06d.png" % det_box1[0])
		frame_path2 = os.path.join(frame2_PATH, "%06d.png" % det_box2[0])
	...
	output_dir = opt.output_dir_root+"/"+opt.video1_name.split('/')[5]+"_"+opt.video1_name.split('/')[7] + " VS " + opt.video2_name.split('/')[5]+"_"+opt.video2_name.split('/')[7]
	#output_dir = opt.output_dir_root+"/" + "--IXKwloYLE_0" + "VS" + "1xWqVxZNf1Q_1"
	...
	sf.write(os.path.join(output_dir, 'audio1_separated.wav'), separation1, opt.audio_sampling_rate)
	sf.write(os.path.join(output_dir, 'audio2_separated.wav'), separation2, opt.audio_sampling_rate)
	...
	if opt.visualize_spectrogram:
        ...
        #them
		#utils.visualizeSpectrogram(pred_mask1[0,:,:], os.path.join(output_dir, 'pred_mask1.png'))
		#utils.visualizeSpectrogram(pred_mask2[0,:,:], os.path.join(output_dir, 'pred_mask2.png'))
		#pred_masks_new_1 = pred_masks1.numpy()
		#pred_masks_new_2 = pred_masks2.numpy()
		print("pred_mask1:/n")
		print(pred_mask1.detach().cpu().numpy().shape)
		print("pred_mask2:/n")
		print(pred_mask2.detach().cpu().numpy().shape)
		import imageio
		#imageio.imwrite(os.path.join(output_dir, 'pred_mask1.png'),np.reshape(pred_mask1.detach().cpu().numpy(), (256,256)).astype(np.uint8))
		#imageio.imwrite(os.path.join(output_dir, 'pred_mask2.png'),np.reshape(pred_mask2.detach().cpu().numpy(), (256,256)).astype(np.uint8))
		#imsave(os.path.join(output_dir, 'pred_mask1.png'),pred_mask1.detach().cpu().numpy())
		#imsave(os.path.join(output_dir, 'pred_mask2.png'),pred_mask2.detach().cpu().numpy())

		imageio.imwrite(os.path.join(output_dir, 'pred_mask1.png'),pred_mask1.detach().cpu().numpy()[0][0][:][:].astype(np.uint8))
		imageio.imwrite(os.path.join(output_dir, 'pred_mask2.png'),pred_mask2.detach().cpu().numpy()[0][0][:][:].astype(np.uint8))
		#import matplotlib.pyplot as plt
		#plt.imsave(os.path.join(output_dir, 'pred_mask1.png'),np.reshape(pred_mask1.detach().cpu().numpy(), (256,256)))
		#plt.imsave(os.path.join(output_dir, 'pred_mask2.png'),np.reshape(pred_mask2.detach().cpu().numpy(), (256,256)))
		#print(np.reshape(pred_mask1.detach().cpu().numpy(), (256,256)))
		#print(np.reshape(pred_mask2.detach().cpu().numpy(), (256,256)))
		print(pred_mask1.detach().cpu().numpy()[0][0][:][:])
		print(pred_mask2.detach().cpu().numpy()[0][0][:][:])
		print(output_dir)
